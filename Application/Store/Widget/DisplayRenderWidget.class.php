        ...
        switch ($data['field']['input_type']) {
            ...
            case IT_PIC:
                ...
                if (intval($data['data']['data'][0]) == 0)
                    return '';
                //单图片
                if (($data['no_hidden']) == 1 && (intval($data['data']['data'][0]) == 0)) {

                }
                else{
                    if($data['jqzoom'])
                    {
                            $content .= '<a target="_blank" class="jqzoom" href="' . get_cover($data['data']['data'][0],'path') . '" title="pic"><img title="点击查看大图"  class="fix" title="pic" src="' . getThumbImageById($data['data']['data'][0],340,'auto') . '"></a>';
                        /*}else{
                            $content .= '<a target="_blank" class="" href="' . getThumbImageById($data['data']['data'][0]) . '" title="pic"><img title="点击查看大图"  class="fix" title="pic" src="' . getThumbImageById($data['data']['data'][0],460,460) . '"></a>';
