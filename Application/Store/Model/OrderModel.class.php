...
class OrderModel extends Model
{
    ...
    function addOrder($order, $good_ids, $good_counts)
    {
        ...
        foreach ($goods as $key => $v) {
            ...
            $shop = D('Shop')->getById($v[0]['shop_id']);
            ...
            //给店主发信息
            D('Message')->sendMessage($shop['uid'], $content = '【微店】订单' . $rs . '已创建，等待买家付款！', $title = '微店下单通知', U('store/center/sold'), is_login());
    ...
    function comput_sum($items)
    {
        ...
        foreach ($items as $k_item => $v_item) {
            $orders['data'][$k]['goods'][] = D('Info')->getById($v_item['good_id']);
    ...
    function pay($order_id)
    {
        ...
        if ($currencyModel->pay($cost, is_login())) {
            ...
            if ($this->save($order)) {
                ...
                //给店主发信息
                D('Message')->sendMessage($shop['uid'], $content = '【微店】订单' . $order['id'] . '已付款，赶紧发货吧！', $title = '微店订单付款通知', U('store/center/sold'), is_login());
                return true;
            } else {
    ...
    function closeOrder($aOrderId)
    {
        ...
        if ($order['uid'] == get_uid()) { //买家关
            ...
            $map['uid'] = get_uid();
            $map['id'] = $aOrderId;
            $rs = $this->where($map)->setField('condition', ORDER_CON_CLOSED);
    ...
    function response($order_id, $response, $content)
    {
        ...
        //状态检测
        if ($order['condition'] != ORDER_CON_DONE) {
            $this->error = '订单状态有误。';
            return false;
        }
        ...
        if ($rs) {
            $shop = $this->getShopFromOrder($order);
            D('Message')->sendMessage($shop['uid'], $content = '【微店】买家已修改评价' . $order['id'] . '！', $title = '微店订单评价通知', U('store/center/sold'), is_login());
            return true;
        } else {
            $this->error = '数据更改错误。';
            return false;
